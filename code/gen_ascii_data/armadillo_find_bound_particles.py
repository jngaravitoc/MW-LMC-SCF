...
if __name__ == "__main__":
    ...
    for i in range(init_snap, final_snap):
        ...
        halo = reads.read_snap_coordinates(in_path, snapname+"{:03d}".format(i), n_halo_part, com_frame='MW', galaxy='MW')
        ...
        satellite = reads.read_snap_coordinates(in_path, snapname+"{:03d}".format(i), n_halo_part, com_frame='sat', galaxy='sat')
        ...
        armadillo = lmcb.find_bound_particles(pos_sat_em, vel_sat_em, mass_sat_em, ids_sat_em, rs, nmax, lmax)
	...
	## TODO: include out_path
        ...
        np.savetxt("unbound"+out_name, mw_lmc_unbound)
        ...
        results = cop.main(pool, pos_host_sat, mass, nmax, lmax, rs, var=True)
        cop.write_coefficients("test_coefficients.txt", Snlm, varSnlm, Tnlm, varTnlm, varSTnlm, nmax, lmax, r_s)
